...
function Command({
  ...
  return (
    <CommandPrimitive
      data-slot="command"
      className={cn(
        "flex size-full min-h-0 flex-col overflow-hidden rounded-2xl text-popover-foreground",
        className
      )}
      {...props}
    />
  ...
  return (
    ...
    >
      ...
      <CommandPrimitive.Input
        ...
        className={cn(
          "flex h-10 w-full rounded-md bg-transparent py-3 text-base outline-none placeholder:text-placeholder disabled:cursor-not-allowed disabled:opacity-64 sm:text-sm",
          className
        )}
        {...props}
      />
...
function CommandList({
  ...
  return (
    <CommandPrimitive.List
      ...
      className={cn(
        "max-h-[min(60vh,24rem)] min-h-0 scroll-fade-y scroll-py-1 overflow-x-hidden overflow-y-auto overscroll-y-contain py-1",
        className
      )}
      {...props}
    />
